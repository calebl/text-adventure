  ...
  def add_chat_and_message_columns
    ...
    with_upgrade_safety do
      change_table :messages, bulk: true do |table|
        ...
        table.json :citations unless column_exists?(:messages, :citations)
        table.json :server_tool_calls unless column_exists?(:messages, :server_tool_calls)
        table.json :raw_content unless column_exists?(:messages, :raw_content)
        table.json :raw_reasoning unless column_exists?(:messages, :raw_reasoning)
      end
    end
  end
