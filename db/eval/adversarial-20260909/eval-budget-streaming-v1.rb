...
module ReviewEvalBudget
  ...
  class << self
    ...
    def usage(response)
      ...
      # Streaming responses can retain an empty body or raw SSE, while the
      # assembled message still has token counts. Only an actual JSON object
      # can carry this optional OpenRouter billing metadata.
      body = body.lstrip.start_with?("{") ? JSON.parse(body) : nil if body.is_a?(String)
