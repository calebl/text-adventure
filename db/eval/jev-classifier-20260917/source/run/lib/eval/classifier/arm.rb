...
class Eval::Classifier::Arm
  # `typesafe` is benchmark-only. It names the System One adapter under
  # `Eval::Classifier`; it is never handed to RubyLLM or added to BaseAgent's
  # live model rotation.
  PROVIDERS = %i[openrouter ollama typesafe].freeze
  ...
  # `"ollama:qwen3:8b"` -> ollama, `qwen3:8b`; `"typesafe:jev-latest"`
  # selects the benchmark-only System One transport. Anything with no known
  # provider prefix is OpenRouter, which is what the app's own ids are.
  ...
  def id = [ (local? || jev?) ? "#{provider}:#{model}" : model,
             thinking_off? ? NOTHINK_SUFFIX : nil ].compact.join

  def thinking_off? = provider_params == NO_THINKING

  def local? = provider == :ollama
  def jev? = provider == :typesafe

  def to_s = id

  def model_options
    raise UnknownProvider, "#{id} uses the System One bench adapter, not RubyLLM" if jev?
  ...
  def price
    return Eval::Cost::UNKNOWN if local?
    return Eval::Classifier::JevAgent::PRICE if jev?

    Eval::Cost.price(model)
  end
  ...
  def pinned
    # Jev never enters BaseAgent's runtime model options. Its explicit arm is
    # read by `Bench#read`, which constructs the Eval-only BaseAgent subclass.
    return yield(self) if jev?
