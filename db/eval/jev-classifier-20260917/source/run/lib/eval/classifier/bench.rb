...
class Eval::Classifier::Bench
  ...
  Reading = Data.define(:line, :arm, :rep, :answer, :answered_by, :raw, :seconds, :error,
                        :confidence, :probabilities, :usage, :billed_cost, :request, :receipt) do
    def initialize(confidence: nil, probabilities: nil, usage: nil, billed_cost: nil,
                   request: nil, receipt: nil, **rest)
      super(confidence:, probabilities:, usage:, billed_cost:, request:, receipt:, **rest)
    end
  ...
  Warmup = Data.define(:arm, :seconds, :residency, :error, :usage, :billed_cost, :request, :receipt) do
    def initialize(usage: nil, billed_cost: nil, request: nil, receipt: nil, **rest)
      super(usage:, billed_cost:, request:, receipt:, **rest)
    end
    def failed? = !error.nil?
    def to_h
      { arm:, seconds: seconds&.round(4), residency: residency.to_s, error:, usage:,
        billed_cost:, request:, receipt: }.compact
    end
  end
  ...
  Pass = Data.define(:arm, :rep, :readings) do
    ...
    # SYSTEM ONE EVIDENCE BESIDE THE ORDINARY SCORE. Confidence is not a gate
    # on the score above: this records what candidate floors would cover, how
    # often they would fall back, and how many wrong choices each floor would
    ...
    def to_h = { arm:, rep:, accuracy: accuracy.round(4), intent_accuracy: intent_accuracy.round(4),
                 ...
                 **also_counts, jev: jev_evidence, readings: rows }.compact
    ...
    def reading_h(reading)
      ...
      { id: reading.id, shape: reading.shape, typed: line.typed, position: line.position,
        ...
        raw: reading.raw, confidence: reading.confidence, probabilities: reading.probabilities, usage: reading.usage,
        billed_cost: reading.billed_cost, request: reading.request, receipt: reading.receipt }.compact
    end

    private
      # Nearest-rank, matching `latency_p95`: every printed quantile is an
      # observed confidence rather than an interpolation the provider returned.
      def quantile(sorted, proportion)
        sorted[[ (sorted.size * proportion).ceil - 1, 0 ].max]
      end
  end
  ...
  def warm(arm)
    ...
    warmup = Warmup.new(arm: arm.id, seconds: seconds, residency: residency, error: error,
                        usage: reading.usage, billed_cost: reading.billed_cost,
                        request: reading.request, receipt: reading.receipt)
    ...
    begin
      if arm.jev?
        ...
        return Reading.new(line: line, arm: arm.id, rep: rep, error: nil, seconds: elapsed,
                           ...
                           usage: result.usage, billed_cost: result.billed_cost,
                           request: result.request, receipt: result.receipt)
      end
