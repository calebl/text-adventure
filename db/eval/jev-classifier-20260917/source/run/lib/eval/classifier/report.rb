...
class Eval::Classifier::Report
  ...
  def jev_confidence(arm)
    ...
    say
    say "  SYSTEM ONE CONFIDENCE -- raw choices stay in the score; floors describe a fallback policy"
    %w[min p25 median p75 p95 max].each do |quantile|
      values = evidence.map { |row| dig_value(row, "confidence", quantile).to_f }
      say format("    %-8s %.3f..%.3f (median %.3f)", quantile, values.min, values.max, Eval.median(values))
    end
    Eval::Classifier::JevAgent::CONFIDENCE_FLOORS.each do |floor|
      key = format("%.1f", floor)
      coverage = evidence.map { |row| dig_value(row, "floors", key, "coverage").to_f }
      wrong = evidence.map { |row| dig_value(row, "floors", key, "wrong").to_i }
      say format("    floor %s  coverage %.3f..%.3f (median %.3f); wrong trusted %d..%d (median %s)",
                 key, coverage.min, coverage.max, Eval.median(coverage),
                 wrong.min, wrong.max, Eval.count(Eval.median(wrong)))
    end
    fallback = evidence.map { |row| indifferent_value(row, "low_confidence_fallback_rate").to_f }
    say format("    fallback @%.1f %.3f..%.3f (median %.3f), before network failures",
               Eval::Classifier::JevAgent::FALLBACK_FLOOR, fallback.min, fallback.max, Eval.median(fallback))
  end

  def indifferent_value(hash, key) = hash[key] || hash[key.to_sym]
  def dig_value(hash, *keys) = keys.reduce(hash) { |memo, key| indifferent_value(memo, key) }
