...
class Eval::Classifier::Result
  # Exact System One request/response receipts live beside the compact score.
  # They are gzip JSONL because request bodies dominate their size. No
  # Authorization header is represented anywhere in a receipt.
  JEV_RECEIPTS = "jev-receipts.jsonl.gz".freeze
  ...
  def write!(directory, name: nil)
    ...
    write_jev_receipts!(dir)
    dir.join(Eval::Classifier::RESULTS)
  end

  # KEEP THE COMPACT FIGURES AND THE RAW JEV RECEIPTS IN ONE OPERATION. Calling
  # `summary.write!` first discards the rows the receipt archive is built from;
  # this method writes the archive from the full result before it writes the
  # compact classifier.json. Ordinary chat-only sets get exactly the historical
  # summary shape and no extra file.
  def write_summary!(directory, name: nil)
    dir = Pathname.new(directory)
    FileUtils.mkdir_p(dir)
    @name ||= name || dir.basename.to_s
    write_jev_receipts!(dir)
    File.write(dir.join(Eval::Classifier::RESULTS), "#{JSON.pretty_generate(summary.to_h)}\n")
    dir.join(Eval::Classifier::RESULTS)
  end

  def for_arm(arm) = passes.select { |pass| pass.arm == arm }

  def jev_evidence(arm) = for_arm(arm).filter_map(&:jev_evidence)

  def actual_usage(arm)
    pass_usage = jev_evidence(arm).map { |evidence| indifferent(evidence, "usage").to_h }
    warm = warmup(arm)&.fetch("usage", nil).to_h
    rows = pass_usage + (warm.present? ? [ warm ] : [])
    {
      input_tokens: rows.sum { |row| indifferent(row, "input_tokens").to_i },
      output_tokens: rows.sum { |row| indifferent(row, "output_tokens").to_i }
    }
  end

  def actual_billed_cost(arm)
    pass_cost = jev_evidence(arm).sum { |evidence| indifferent(evidence, "billed_cost").to_f }
    pass_cost + warmup(arm).to_h.fetch("billed_cost", 0).to_f
  end
  ...
  def write_jev_receipts!(directory)
    arm_ids = arms.select { |arm| Eval::Classifier::Arm.parse(arm).jev? }
    return if arm_ids.empty?

    Zlib::GzipWriter.open(directory.join(JEV_RECEIPTS).to_s) do |gzip|
      warmups.select { |row| arm_ids.include?(row["arm"].to_s) }.each do |row|
        gzip.puts JSON.generate({ kind: "warmup", rep: 0, arm: row["arm"],
                                  request: row["request"], receipt: row["receipt"],
                                  usage: row["usage"], billed_cost: row["billed_cost"],
                                  error: row["error"] }.compact)
      end
      passes.select { |pass| arm_ids.include?(pass.arm) }.each do |pass|
        pass.rows.each do |original|
          row = original.transform_keys(&:to_s)
          gzip.puts JSON.generate({ kind: "reading", rep: pass.rep, arm: pass.arm,
                                    id: row["id"], expected: row["expected"], got: row["got"], raw: row["raw"],
                                    right: row["right"], seconds: row["seconds"],
                                    confidence: row["confidence"], probabilities: row["probabilities"],
                                    usage: row["usage"], billed_cost: row["billed_cost"],
                                    request: row["request"], receipt: row["receipt"],
                                    error: row["error"] }.compact)
        end
