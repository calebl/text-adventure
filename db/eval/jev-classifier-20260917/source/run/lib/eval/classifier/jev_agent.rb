...
#
# ONE CHOICE HOLDS THE WHOLE ANSWER. Separate intent and target questions can
# disagree, while a free-form value would reopen the record boundary. Each
# opaque choice therefore means one complete, engine-valid
...
#
# REJECTED ENCODINGS:
# - one Choice for intent and another for target: dependent answers can conflict;
...
class Eval::Classifier::JevAgent < BaseAgent
  ...
  class Transport
    def post(payload, api_key:, timeout: TIMEOUT)
      ...
      raise HttpError, "System One returned HTTP #{receipt.status}" unless response.is_a?(Net::HTTPSuccess)

      receipt
    rescue JSON::ParserError
      raise MalformedResponse, "System One returned non-JSON"
    end
  end

  attr_reader :classifier, :timeout, :transport
  ...
  def classify(command)
    ...
    Result.new(
      ...
      usage: usage,
      request: request,
      receipt: { "status" => receipt.status, "request_id" => receipt.request_id, "body" => receipt.body },
  ...
  private
    ...
    def validate(body, options)
      ...
      raise MalformedResponse, "System One response did not match the requested Choice" unless valid
