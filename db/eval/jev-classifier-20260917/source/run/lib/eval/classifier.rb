...
module Eval::Classifier
  ...
  # Jev receives the current classifier instructions plus one criterion per
  # complete tuple, so its request is larger. This is only the pre-run spend
  # estimate; the result and board price the provider's actual usage receipts.
  JEV_PER_CALL = { input: 3_500, output: 300 }.freeze
  ...
  def self.estimate(lines:, reps:, models:)
    ...
    Arm.all(models).sum do |arm|
      usage = arm.jev? ? JEV_PER_CALL : PER_CALL
      arm.price.of(calls * usage[:input], calls * usage[:output])
    end
  end
end
