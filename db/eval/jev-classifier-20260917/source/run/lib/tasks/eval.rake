...
namespace :eval do
  ...
  module ClassifierTasks
    ...
    # A bench run is cents, not dollars -- the current corpus x four reps x both
    # live models is under the ceiling below -- so the estimate is still printed first: the
    # captain's rule for `eval:run` applies to anything that spends.
    SPEND_CEILING = 0.50
    ...
    def run!
      ...
      puts format("ESTIMATE: %d calls (%d lines x %d reps x %d model%s), about $%.3f.",
                  calls, corpus.size, reps, arms.size, arms.one? ? "" : "s", priced)
      puts "Chat arms use the measured #{Eval::Classifier::PER_CALL[:input]} in / " \
           "#{Eval::Classifier::PER_CALL[:output]} out per-call estimate; Jev uses " \
           "#{Eval::Classifier::JEV_PER_CALL[:input]} in / #{Eval::Classifier::JEV_PER_CALL[:output]} out, " \
           "then records actual billable usage."
    ...
    def abort_without_a_key(arms)
      jev = arms.select(&:jev?)
      if jev.any? && ENV["TYPESAFE_API_KEY"].blank?
        abort "TYPESAFE_API_KEY is not set and #{jev.map(&:id).join(", ")} is a System One bench arm. " \
              "The key is read only from this process environment."
      end

      hosted_chat = arms.reject { |arm| arm.local? || arm.jev? }
      if hosted_chat.any? && ENV["OPENROUTER_API_KEY"].blank?
        abort "OPENROUTER_API_KEY is not set and #{hosted_chat.map(&:id).join(", ")} " \
              "#{hosted_chat.one? ? "is" : "are"} hosted. Name local arms instead " \
