# TWO REALIZATION BENCH RUNS, WITH A VERDICT PER FIGURE: REAL, NOISE or
# INCONCLUSIVE.
#
...
# separation looks.
#
# WHAT IT IS FOR, and it is the whole reason this bench exists: a realization
# prompt change is judged HERE first and confirmed by `rake eval:run` after. The
# two sets differ in exactly one of two ways, and the printer says which:
#
#   TWO PROMPT VERSIONS ON ONE MODEL -- the ordinary before/after of a change to
#   `Location::Generator`'s people, items or exits instructions. Same arm both
#   sides, same `corpus_digest`, different `prompt_digest`.
#
...
# loudly above the verdicts rather than left to be worked out.
#
# A DIFFERENT CORPUS IS WORSE THAN EITHER, and gets the same treatment: two sets
# that built different rooms are not comparable and the difference between them
# is a difference between two files.
#
# PER ARM, NEVER POOLED. Two models are two measurements.
class Eval::Realization::Comparison
  Row = Data.define(:arm, :metric, :verdict) do
    def neutral? = Eval::Realization::Result::NEUTRAL.include?(metric)

    def improved?
      Eval::Realization::Result.lower_is_better.include?(metric) ? verdict.delta.negative? : verdict.delta.positive?
    end

    def direction
      # THE COUNTS AND THE SPEND HAVE NO BETTER DIRECTION, and saying so is the
      # point of printing them: the cheapest way to clear every rate above is to
      # write one exit and nobody, so a fall here beside a fall in every rate is
      # a prompt that bought its numbers with emptier rooms. See
      # `Eval::Realization::Scorer`.
  ...
  def verdicts(arm, against: arm)
    Eval::Realization::Result.metrics.keys.map do |metric|
  ...
  def print
    say Eval::Realization::Report::RULE
    say "REALIZATION BENCH: #{before.name} -> #{after.name}"
    say "Exact rank test at p <= #{Eval::Noise::ALPHA}, #{Eval::Noise::MIN_RUNS} repetitions a side minimum."
    say Eval::Realization::Report::RULE
    provenance(before)
    provenance(after)
    say "Latencies are WARM-CACHE figures -- each arm's first realization is timed apart and excluded."
    ...
    pairs.each do |against, arm|
      ...
      verdicts(arm, against: against).each do |row|
        verdict = row.verdict
        say format("  %-28s %s -> %s  %-9s %s", row.metric,
  ...
  def warnings
    unless comparable_corpus?
      say
      say "WARNING: THE TWO SETS BUILT DIFFERENT ROOMS (#{before.corpus_digest} vs #{after.corpus_digest})."
  ...
  def figure(metric, value)
    return format("%.2fs", value) if Eval::Realization::Result::SECONDS.include?(metric)
    return format("%d", value) if Eval::Realization::Result::COUNTED.include?(metric)
    return format("%.2f", value) if Eval::Realization::Result::MEANS.include?(metric)
  ...
  def closing
    say
    say "A REAL verdict on a CHECK is what means a prompt change worked. Read the reported counts"
    say "beside it: a fall in every rate with a fall in `exits_named`, `new_places_opened` or"
    say "`people_named` is a prompt that built emptier rooms, which is the one way to improve"
    say "these numbers without improving the game (Eval::Realization::Scorer)."
