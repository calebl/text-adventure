# Derived from the retained NPC study budget helper; the task ceiling is
# LIMIT_MICROS. Unknown requests keep their full reservations. All
...
module Eval::Arrival::Budget
  ...
  class Ledger
    ...
    def reserve!(label:, input_cap:, output_cap: MAX_OUTPUT_TOKENS)
      ...
      edit do |data|
        ...
        raise Halt, "Arrival $2 evaluation budget cannot reserve the next call" if charged + reservation > LIMIT_MICROS
    ...
    def snapshot
      edit { |data| JSON.parse(JSON.generate(data)) }
    end
  ...
  class << self
    ...
    def usage(response)
      ...
      metadata_error = nil
      # Streaming responses can retain an empty body or raw SSE. Their message
      # token counts above remain usable even if optional raw billing JSON is
      # incomplete; preserve that parse error as metadata, not a game failure.
      if body.is_a?(String)
        begin
          body = body.lstrip.start_with?("{") ? JSON.parse(body) : nil
        rescue JSON::ParserError => error
          metadata_error = error.class.name
          body = nil
        end
      end
  ...
  module AgentCalls
    def ask(prompt, **options, &block)
      ...
      receipt = { purpose: purpose, prompt: prompt, instructions: instructions, schema: schema_json, history: request[:history],
                  requested_model: MODEL, provider: PROVIDER, input_cap: input_cap,
                  output_cap: MAX_OUTPUT_TOKENS, registry_rates: price }
      ticket = Eval::Arrival::Budget.ledger.reserve!(label: "#{Eval::Arrival::Budget.label}:#{purpose}", input_cap: input_cap)
      Eval::Arrival::Budget.calls << receipt
      Thread.current[:arrival_eval_receipt] = receipt
      ...
      begin
        ...
        Thread.current[:arrival_eval_receipt] = nil
        Eval::Arrival::Budget.ledger.settle!(ticket, receipt)
      end
    end
  end
  ...
  module ChatCalls
    def ask(*args, **options, &block)
      result = super
      receipt = Thread.current[:arrival_eval_receipt]
      if receipt
        receipt.merge!(Eval::Arrival::Budget.usage(result))
