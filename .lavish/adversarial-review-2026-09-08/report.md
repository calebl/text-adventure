...
NPC decisions now apply validated game effects, arrivals receive current records, and turns are serialized and recover safely from rendering failures. The recovered reviewed source at 4b7235b passes 4,154 tests and the real-browser fixture checks. Live NPC cases apply their promised state changes; arrival prose still sometimes contradicts inventory, so that finding remains partial. Deterministic notices expose crossing costs even when narration omits them.

Original review and measured baseline: 8dd1f5c. Focused model candidate: 93448ade; sequential confirmation: 57f8738f adds the deterministic toll view without changing model input. These commits identify frozen measurements, not later code. Main 2ef1a20 was integrated at 337f826; subsequent review fixes culminated in 83f56d8. Recovery merge 4b7235b retains both histories and has exactly the same Git tree as 83f56d8. The latest full suite below describes that recovered source; the pull request checks describe its published head.
