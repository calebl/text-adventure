...
NPC decisions now apply validated game effects, arrivals receive current records, and turns are serialized and recover from rendering failures. Final source 903269c passes 4,155 tests, lint, autoloading and the security scan. Live NPC cases apply their promised state changes; arrival prose still sometimes contradicts inventory, so that finding remains partial. Deterministic notices expose crossing costs even when narration omits them.

Original review and measured baseline: 8dd1f5c. Focused model candidate: 93448ade; sequential confirmation: 57f8738f adds the deterministic toll view without changing model input. These commits identify frozen measurements, not later code. Main 2ef1a20 was integrated at 337f826; subsequent review fixes culminated in 83f56d8. Recovery merge 4b7235b retains both histories and has exactly the same Git tree as 83f56d8. The latest full suite below describes that recovered source; the pull request checks describe its published head. Final source 903269c corrects the optional debug panel to use per-game whereabouts and makes an existing quest test explicitly supply population capacity. Model inputs and engine behavior are unchanged from the browser-tested source. Delivery-tool failures and the final independent checks are recorded below; neither failed pipeline is called passed.

Original evidence is preserved in [the original report](original-report.md).

## Verification

- **Final source and delivery verification (903269c):** 4,155 tests / 23,359 assertions; zero failures, errors or skips, including sweeps. RuboCop, Zeitwerk and Brakeman passed. A before/after regression verifies the debug follower fix; the previously failing quest fixture now supplies explicit population capacity. Both delivery-tool failures and the second completed browser pass are archived without mislabelling the pipeline outcome. [Evidence](delivery-verification.md)
- **Recovered reviewed source (4b7235b):** 4,154 tests / 23,400 assertions; zero failures, errors or skips, including engine sweeps. RuboCop: 556 files, no offenses. Zeitwerk passed. The recovery merge has exactly the same Git tree as reviewed 83f56d8; only report/evidence additions follow these checks. [Evidence](verified-checks-4b7235b.md)
- **Real app and worker in Chromium (83f56d8):** Actual bin/dev app and Solid Queue worker with isolated databases and a local deterministic provider. Verified NPC gifts, ceasefire and following, malformed/stale choices, arrival facts and visible tolls, duplicate/overtaken/queued submissions, renderer outage fallback, checkpoint resume and missing-model notices. No paid calls. The agent reported no findings, but the old pipeline timed out before returning and never pushed; this archives the completed browser checks without calling that pipeline passed. [Evidence](browser-validation/README.md)
...
- The optional debug panel now reads per-game whereabouts too: a present follower is not labelled absent, and a later departure is reported. The regression failed before and passes after the correction.
